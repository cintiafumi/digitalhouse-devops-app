            ...
            steps { 
                script {
                    if(env.GIT_BRANCH=='origin/homolog'){
                        ...
                        sh "docker run -d --name app1 -p 8030:3000 -e NODE_ENV=homolog -e AWS_ACCESS_KEY=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e 
                            _NAME=digitalhouse-grupo2-homolog 402303658132.dkr.ecr.us-east-1.amazonaws.com/devops:latest"
                        }
